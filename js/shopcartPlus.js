...
$(function () {
    ...
    /*全选 */
    /*
       1、给表头中的全选按钮绑定点击事件， 点击的时候获取到它的选中状态（true/false）
       2、给表格中每一行的数据的input的选择框，赋值为表头的选中状态（true/false）
    ...
    */
    ...
    /*
       计算总价的全选
           1、给计算总价的全选按钮 绑定点击事件，获取选择状态（tuer/false）
           2、把状态给表头的全选
           3、把状态给表格的选择框
    */
    $totalPriceInput.change(function () {
        var checkState = $(this).prop('checked');   //获取计算总价中的全选状态
        $theadInput.prop('checked', checkState);     //赋值给表头中全选框
    ...
    */
    $('.add').click(function () {    //给增加绑定事件
        ...
        count++;   //自增
        $(this).next().val(count); //把后面的值赋值给输入框
        //小计
        subTotal($(this), count);
        allTotal();   //总计
    })
    ...
    */
    ...
    /*
         总计功能实现（头部全选 计算总价全选 表格的选择框+ -删除 六个方向调用总计）
         1、定义一个变量 用于保存总价 定义一个变量 用于保持已选商品 数量
         2、获取所有表格中的选择框，循环，获取选中状态，判断
         3、如果选中，那么就要累加这一行的小计
    */
    function allTotal() {
        var allTotal = 0;    //定义一个变量 用于保存总价
        var selectedCount = 0;    //定义一个变量 用于保持已选商品 数量
        $('tbody input[type=checkbox]').each(function () {  //获取表格中的选择框 循环
            var checkState = $(this).prop('checked');  //获取选中状态
            if (checkState) {  //如果是true
                allTotal += parseFloat($(this).closest('tr').find('.subprice').text()); //累加这一行的小计
                selectedCount++;  //数量+1
            }

        })
        //渲染
        $('.total').text(allTotal.toFixed(2));  //渲染总价
    ...
    */
    ...
    function getGoodsCount() {
        //获取数量
        var goodsConut = $('table tbody tr').length;
        //渲染
        $('.goodsCount').text(goodsConut);
    }
